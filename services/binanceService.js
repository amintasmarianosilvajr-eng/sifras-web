...
class BinanceService {
    ...
    async initBlacklist() {
        try {
            ...
            console.error("[BINANCE] Falha na Blacklist:", e.message);
        }
    }
    ...
    async syncRanking() {
        ...
        try {
            ...
            console.error("[BINANCE] Falha no Ranking:", e.message);
        } finally {
            this.isSyncing = false;
        }
    }
    ...
    async startGlobalWS() {
        ...
        this.ws.on("close", () => setTimeout(() => this.startGlobalWS(), 5000));
    }
    ...
    async executeOrder(key, secret, symbol, side, qty) {
        ...
        const info = await axios.get(`https://api.binance.com/api/v3/exchangeInfo?symbol=${symbol}`);
